class NuiSkeletonData
{
    ...
    void addFrame ( String data )
    {
        ...
        String[] pieces = data.replace(" ","").split(",");
        ...
        frame[i] = Integer.parseInt( pieces[0] );
        validity[i] = Integer.parseInt( pieces[2] );
        timestamp[i] = Integer.parseInt( pieces[3] );
        
        for ( int s = 0, n = 4; n < pieces.length && s < skeleton.length; n+=4, s++ )
        {
            skeleton[s][i] = new Point3D(
                Float.parseFloat(pieces[n]),
                Float.parseFloat(pieces[n+1]),
                Float.parseFloat(pieces[n+2])
            );
        }
    }
